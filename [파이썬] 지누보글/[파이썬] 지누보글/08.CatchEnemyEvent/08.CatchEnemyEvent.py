...
class CatchEnemy:
    def __init__(self):
        self.window = Tk()
        self.window.title("지누 버블 - 적 버블에 가두기 연습")
        ...
        self.jinu = self.canvas.create_oval(
            360, 600, 400, 640, fill="lightgreen", outline="green", width=3
        )
        self.direction = 1
    ...
    def catch_enemy(self, enemy):
        ex1, ey1, ex2, ey2 = self.canvas.coords(enemy)

        self.normal_enemies.remove(enemy)
        self.canvas.delete(enemy)

        trapped = self.canvas.create_oval(
            ex1, ey1, ex2, ey2,
            fill="skyblue", outline="blue", width=2
        )

        self.trapped_enemies.append(trapped)



    def update_trapped_enemies(self):
        for t in self.trapped_enemies[:]:
            self.canvas.move(t, 0, -2)   

            x1, y1, x2, y2 = self.canvas.coords(t)

            if y2 < 0: 
                self.canvas.delete(t)
                self.trapped_enemies.remove(t)
